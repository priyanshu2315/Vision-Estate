...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success === false) {
        dispatch(updataUserFailure(data.message));
        return;
      }
      dispatch(updateUserSucces(data));
      setUserUpdated(true);
    } catch (error) {
      dispatch(updataUserFailure(error.message));
    }
  };
  ...
  const handleDeleteUserListings = async (id) => {
    try {
  ...
  return (
    <div className="max-w-lg mx-auto p-3 ">
      ...
      <form className="flex flex-col gap-4 mt-5" onSubmit={handleSubmit}>
        ...
        <button
          disabled={loading}
          className="uppercase bg-slate-700 text-white p-3 rounded-lg hover:opacity-90 disabled:opacity-80"
        >
          ...
          {userListings.map((listing) => (
            ...
            >
              ...
              <div className="flex flex-col items-center">
                <button
                  onClick={() => handleDeleteUserListings(listing._id)}
                  className="text-red-700 uppercase text-sm font-semibold"
                >
                  Delete
                </button>
                <Link to={`/update-listing/${listing._id}`}>
                  <button className="text-green-700 uppercase text-sm font-semibold">
                    Edit
                  </button>
                </Link>
              </div>
